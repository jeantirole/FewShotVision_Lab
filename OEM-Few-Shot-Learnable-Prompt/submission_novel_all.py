...
if __name__ == '__main__':
    ...
    ckpt = T.load(osp.join(args.ckpt_path, 'all_test.pt'), map_location='cpu')
    ...
    print(f'Running inference for novel class all')
    print("args.dataset_dir ", args.dataset_dir)
    for file in tqdm(os.listdir(args.dataset_dir)):
        
        outdir = osp.join(args.outdir, f'novel_all')
        i = "all_novel"
        split_ = 2 #=> label 마다 split 값 따로 가져갔는데. 일단 그냥 통일 ! 
        NovelInference.inference_image_with_crop(novel_model, args.device, os.path.join(args.dataset_dir, file), i, outdir=outdir, split=split_)
